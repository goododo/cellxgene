...
args = sys.argv
if len(args) > 1:
    dataset_id_lists = args[1:]
    print(f"The dataset_id_lists are: {dataset_id_lists}")
else:
    ...
    with ThreadPoolExecutor(2) as t:  # 添加线程池
        for item in data_ls:
            ...
            if (data_key in difference_list):
                ...
                first_id = item['id']
                dataset_id = item['dataset_assets'][0]['dataset_id']
                Assay = "，".join([label['label'] for label in item['assay']])
                cell_count = item['cell_count']
                cell_type = "，".join([label['label'] for label in item['cell_type']])
                development_stage = "，".join([label['label'] for label in item['development_stage']])
                disease = "，".join([label['label'] for label in item['disease']])
                organism = "，".join([label['label'] for label in item['organism']])
                published_at = item['published_at']
                date = datetime.datetime.fromtimestamp(published_at)
                self_reported_ethnicity = "，".join([label['label'] for label in item['self_reported_ethnicity']])
                sex = "，".join([label['label'] for label in item['sex']])
                suspension_type = item['suspension_type'][0]
                tissue = "，".join([label['label'] for label in item['tissue']])
                data = {
                    'Download_ID': [count],
                    'data_key': [data_key],
                    'Dataset': [dataset],
                ...
                download_id = anntation['Download_ID'][anntation['data_key'] == data_key].to_string(index=False).strip()
                first_id = item['id']
                dataset_id = item['dataset_assets'][0]['dataset_id']
                Assay = "，".join([label['label'] for label in item['assay']])
                cell_count = item['cell_count']
        ...
        df.to_csv('download-anntation-new.csv', index=False, sep="\t", quoting=csv.QUOTE_NONE,escapechar='\\')
        if dataframes2:
            df2 = pd.concat(dataframes2, ignore_index=True)
            df2.to_csv('supp_data_url.txt', index=False, sep="\t", quoting=csv.QUOTE_NONE,escapechar='\\')
        #return df
...
dataset_id_lists = args[1:]
anntation_list = []
for dataset_id_list in dataset_id_lists:
    temp_df = pd.read_csv(dataset_id_list, delimiter="\t")
    anntation_list.append(temp_df)
anntation = pd.concat(anntation_list, ignore_index=True)
...
df.to_csv("difference_list.csv", index=False)
